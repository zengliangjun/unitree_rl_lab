...
class RewardsCfg:
    ...
    class LegRewardsCfg:
        ...
        deviation_legs = RewTerm(
            func=mdp.joint_deviation_l1,
            weight=-1.0,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_roll_joint", ".*_hip_yaw_joint", ".*_ankle_roll_joint"])},
        )
        deviation_stand = RewTerm(
            func=mdp.stand_deviation_l1,
            weight=-1.0,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=["waist.*", ".*_hip.*", ".*_knee.*", ".*_ankle.*"])},
        )
    ...
    class ArmRewardsCfg:
        ...
        deviation_stand = RewTerm(
            func=mdp.stand_deviation_l1,
            weight=-1.0,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_shoulder.*", ".*_elbow.*", ".*_wrist.*"])},
        )
        # -- other
        undesired_contacts = RewTerm(
            func=mdp.undesired_contacts,
