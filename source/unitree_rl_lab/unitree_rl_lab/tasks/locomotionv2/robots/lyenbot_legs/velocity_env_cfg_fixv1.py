...
from unitree_rl_lab.assets.robots.lyenbot_legs_48_knee3 import LYENBOT_CFG as ROBOT_CFG
...
class CommandsCfg:
    ...
    base_velocity = commands.CommandWithPhaseCfg(
        ...
        limit_ranges=commands.CommandWithPhaseCfg.Ranges(
            lin_vel_x=(-0.8, 0.8), lin_vel_y=(-0.4, 0.4), ang_vel_z=(-0.3, 0.3)
        ),
...
class RewardsCfg:
    ...
    track_ang_vel_z = RewTerm(
        func=rewards.track_ang_vel_z_world_exp, weight=1.95, params={"command_name": "base_velocity", "std": math.sqrt(0.25)}
    )
    ...
    action_rate = RewTerm(func=mdp_rewards.action_rate_l2, weight=-0.025)
    ...
    # -- robot
    flat_x = RewTerm(func=rewards_ext.flat_orientation_x, weight=-5.0)
    flat_y = RewTerm(func=rewards_ext.flat_orientation_y, weight=-1.5)
    ...
    penalty_contact_orientation = RewTerm(
        func=rewards_ext.penalty_contact_orientation,
        weight=-1.5,
        params={
            "std": 0.05,
            "sensor_cfg": SceneEntityCfg(
    ...
    feet_slide_ang = RewTerm(
        func=rewards_ext.feet_slide_ang,
        weight=-0.25,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*ankle_roll.*"),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*ankle_roll.*"),
    ...
    feet_width = RewTerm(
        func=rewards_ext.reward_feet_widthv2,
        weight=0.3,
        params={
            "asset_cfg":
...
from unitree_rl_lab.assets.robots import lyenbot_legs_48_knee3_delay

@configclass
class RobotDelayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.robot = lyenbot_legs_48_knee3_delay.LYENBOT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")

@configclass
class RobotDelayPlayEnvCfg(RobotPlayEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.robot = lyenbot_legs_48_knee3_delay.LYENBOT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
